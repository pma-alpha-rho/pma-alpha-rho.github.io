...
$sql = "SELECT * FROM events WHERE (DATEDIFF(date,curdate())<2 AND DATEDIFF(date,curdate())>0);";
...
while($upcoming_events = mysql_fetch_array($result)) {
	...
	while($upcoming_opportunities = mysql_fetch_array($result1)) {
		$sql2 = "SELECT * FROM registration WHERE opp_id='{$upcoming_opportunities['opp_id']}' AND approved>0 AND reminder_sent < 1;";
		...
		while($registrants = mysql_fetch_array($result2)) {
			...
			$mail->msgHTML("{$registrant['name']},<br><br>This is just a reminder that you signed up to work at {$upcoming_events['event_name']} tomorrow.  The event begins at $start_time and ends at $end_time.  Please come prepared for the event at the designated time.<br><br> {$upcoming_events['desc']}<br>OAS AAS LLS,<br>Brotrak");
